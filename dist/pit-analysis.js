...
export function mountPitAnalysis(host,{data,evidence,race,showStory=true}){
 ...
 <p class="pit-intro">从这一圈开始比较：留在赛道、本圈结束进站，或再等几圈。只模拟 ${esc(context.driver)} 的时间成本；不预判 ${esc(evidence.second.driver.name_acronym)} 的反应。</p>
 ...
 </details><p class="pit-assumption">橙色参数均为假设。进站损失包含进出站与停车；不会把对手资料缺口填成预测。</p>
 ...
 function render(){
  ...
  q('#pit-breakdown').innerHTML=`<div class="pit-cost-heading"><h3>为什么得出这个差值？</h3><span>${rows.find(r=>r[0]===key)[1]}${!selected.compatible?' · 仅作时间参照':''}</span></div><div class="pit-costs"><div><span>轮胎带来的${tyreGain>=0?'节省':'损失'}</span><strong class="${tyreGain>=0?'gain':'loss'}">${Math.abs(tyreGain).toFixed(2)} s</strong><div class="pit-cost-track"><i style="width:${Math.min(100,Math.abs(tyreGain)/Math.max(1,Math.abs(tyreGain),selected.pitCost,selected.coldCost+selected.trafficCost)*100)}%"></i></div></div><div><span>额外进站损失</span><strong>${selected.pitCost.toFixed(2)} s</strong><div class="pit-cost-track"><i class="pit-cost" style="width:${selected.pitCost/Math.max(1,Math.abs(tyreGain),selected.pitCost,selected.coldCost+selected.trafficCost)*100}%"></i></div></div><div><span>升温 + 交通损失</span><strong>${(selected.coldCost+selected.trafficCost).toFixed(2)} s</strong><div class="pit-cost-track"><i class="pit-cost" style="width:${(selected.coldCost+selected.trafficCost)/Math.max(1,Math.abs(tyreGain),selected.pitCost,selected.coldCost+selected.trafficCost)*100}%"></i></div></div><div><span>合计时间差</span><strong class="${selected.delta<0?'gain':'loss'}">${signed(selected.delta)} s</strong><small>进站 + 升温 + 交通 − 轮胎节省</small></div></div><p class="pit-meaning">${selected.delta<0?'省下的时间是潜在追赶空间。':'付出的时间需要其他收益才能补偿。'} 交通只按你设的额外损失计入。赛道位置、对手下一步和真实剩余胎组仍须核实，净省时间不等于一定能超车。</p>`;
  drawChart();
 }
 function drawChart(){
