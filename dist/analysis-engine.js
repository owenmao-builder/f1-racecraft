...
function driverContext(data,driver,t){
...
// Single-car tyre evidence has no opponent or corner prerequisite.
export function analyseDriver(data,{driver,lap}) {
 const selected=data.drivers.find(d=>d.name_acronym===driver);
 if(!selected)throw Error('本场参赛名单中未找到这位车手。');
 if(!Number.isInteger(lap)||lap<1)throw Error('请选择有效的圈次。');
 const row=data.laps.find(l=>l[0]===selected.driver_number&&l[1]===lap);
 if(!row||!number(row[2]))throw Error(`${driver} 在第 ${lap} 圈没有可用起圈记录，可能已退赛、被套圈或数据缺失。请换一个圈次。`);
 const time=row[2];
 return {first:driverContext(data,selected,time),time,control:(data.control??[]).filter(c=>c[0]<=time&&c[0]>=time-120).slice(-3),request:{driver,lap}};
}
export function analyse(data, request) {
 ...
 const first=driverContext(data,a,t),second=driverContext(data,b,t);
