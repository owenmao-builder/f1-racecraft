...
export function mountObservatory(host,{race,races,selection,moment,changeRace,research}){
 let disposed=false,story=null,lesson='tyres',loading=false;
 const q=s=>host.querySelector(s);
 host.innerHTML=`<div class="observe-brief"><div><span class="eyebrow lime">THE STRATEGIST'S NOTEBOOK</span><h2>先看懂，再做决定。</h2><p>一次只讲一个问题。沿着时间账本，听懂策略师在权衡什么。</p></div><label>跟随哪场比赛<select id="observe-race">${races.map(r=>`<option value="${r.id}" ${r.id===race.id?'selected':''}>${r.short} · ${r.date}</option>`).join('')}</select></label></div>
 ...
 async function loadLesson(){
  ...
  try{
   ...
   if(!context)throw Error('这一时刻没有适用的干胎教学样本，请切换比赛。');
